...
e.cv.onpointerdown=ev=>{if(!base)return;drag=true;e.cv.setPointerCapture(ev.pointerId);const p=pt(ev);draft={x:p.x,y:p.y,w:0,h:0,sx:p.x,sy:p.y};draw()};e.cv.onpointermove=ev=>{if(!drag||!draft)return;const p=pt(ev);draft.x=Math.min(draft.sx,p.x);draft.y=Math.min(draft.sy,p.y);draft.w=Math.abs(p.x-draft.sx);draft.h=Math.abs(p.y-draft.sy);draw()};function end(ev){if(!drag||!draft)return;drag=false;try{e.cv.releasePointerCapture(ev.pointerId)}catch{}if(draft.w>4&&draft.h>4)rects.push({x:draft.x,y:draft.y,w:draft.w,h:draft.h});draft=null;draw()} e.cv.onpointerup=end;e.cv.onpointercancel=end;e.undo.onclick=()=>{rects.pop();draw()};e.clear.onclick=()=>{rects=[];draw()};
